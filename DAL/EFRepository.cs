...
namespace DAL
{
    public class EFRepository : IRepository
    {
        ...
        public bool Delete<TEntity>(TEntity toDelete) where TEntity : class
        {
            ...
            try
            {
                _dbContext.Entry<TEntity>(toDelete).State = EntityState.Detached;
        ...
        public List<TEntity> ListAll<TEntity>() where TEntity : class
        {
            List<TEntity> result = null;
            try
            {
                result = _dbContext.Set<TEntity>().ToList(); // Recupera todos los registros de la entidad
            }
            catch (Exception)
            {
                throw;
